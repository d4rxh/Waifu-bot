...
async def handle_messages(client: Client, message: Message):
    ...
    if chat_id in active_spawns and not active_spawns[chat_id]['claimed']:
        ...
        if message.text.lower().strip() == waifu['name'].lower().strip():
            ...
            db.add_waifu_to_user(message.from_user.id, message.from_user.username, waifu)
            
            rarity_emoji = get_rarity_emoji(waifu['rarity'])
            await message.reply(
                f"🎉 **Congratulations!**\n\n"
                f"{rarity_emoji} @{message.from_user.username} claimed **{waifu['name']}**!\n"
                f"📺 From: `{waifu['anime']}`\n"
...
async def my_waifus_command(client: Client, message: Message):
    ...
    response = f"🎌 **{message.from_user.first_name}'s Collection**\n\n"
...
💬 *"{waifu.get('quote', 'A beloved character!')}"*"""
    ...
    try:
        await message.reply_photo(photo=waifu['image'], caption=info)
    except:
...
async def daily_command(client: Client, message: Message):
    ...
    db.add_waifu_to_user(user_id, message.from_user.username, waifu)
...
@app.on_message(filters.command("forcespawn") & filters.user("admins"))
